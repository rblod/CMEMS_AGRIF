...
# endif
   ...
   SUBROUTINE nemo_init
      ...
      NAMELIST/namctl/ sn_cfctl,  nn_print, nn_ictls, nn_ictle,             &
         ...
         &             ln_timing, ln_diacfl, ln_debug_write, ln_debug_read
...
904   IF( ios >  0 )   CALL ctl_nam ( ios , 'namcfg in configuration namelist' )   
      ...
      IF( ln_debug  .AND. Agrif_Root() )   CALL debug_ini
    !
...
!                           CALL dia_ptr_init    ! Poleward TRansports initialization
   ...
   SUBROUTINE nemo_ctl
      ...
      IF( .NOT. Agrif_Root() ) THEN
         ln_debug_write = Agrif_Parent(ln_debug_write)
         ln_debug_read =  Agrif_Parent(ln_debug_read)
      ENDIF   
#endif
      ln_debug = ln_debug_write .OR. ln_debug_read
      ...
      IF(lwp) THEN                  ! control print
         ...
         WRITE(numout,*) '      write debug informations   ln_debug_write  = ', ln_debug_write
         WRITE(numout,*) '      read  debug informations   ln_debug_read   = ', ln_debug_read
      ENDIF
      !
      ...
      IF( ln_debug_write .AND. ln_debug_read )   CALL ctl_stop('nemo_ctl: choose between ln_debug_write and ln_debug_read')
