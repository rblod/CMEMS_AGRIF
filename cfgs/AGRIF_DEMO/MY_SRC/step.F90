...
# endif
...
#if defined key_si3
      IF( ln_debug ) THEN
         CALL check_tab(u_ice(:,:),'u_ice   after')
         CALL check_tab(v_ice(:,:),'v_ice   after')
         DO jk = 1, jpl
            CALL check_tab(a_i(:,:,jk),'ice area   after')
         END DO
      ENDIF
#endif
      ...
      IF( ln_debug ) THEN
         CALL check_tab(uu(:,:,:,Naa),'u   after')
         CALL check_tab(vv(:,:,:,Naa),'v   after')
         CALL check_tab(ssh(:,:, Naa),'ssh after')
      ENDIF
      ...
      IF( ln_debug ) THEN
         CALL check_tab(ts(:,:,:,jp_tem,Naa),'temp   after')
         CALL check_tab(ts(:,:,:,jp_sal,Naa),'salt   after')
      ENDIF
